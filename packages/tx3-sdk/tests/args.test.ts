  ...
  switch (a.type) {
    ...
    case "UtxoSet": {
      ...
      return setA.every((utxo, i) => {
        const utxoB = setB[i];
        return (
          utxo.ref.index === utxoB.ref.index &&
  ...
  describe("Address Tests", () => {
    ...
    test("round trip address from hex string", () => {
      ...
      jsonToValueTest(
        hexAddress,
        Type.Address,
        createAddressArg(expectedBytes),
      );
    });
  });
  ...
  describe("ArgValue Factory Functions Tests", () => {
    ...
    describe("from (generic)", () => {
      ...
      test("throws error for unsupported type", () => {
        ...
        }).toThrow("Cannot convert value to PrimitiveArgValue");
      });
